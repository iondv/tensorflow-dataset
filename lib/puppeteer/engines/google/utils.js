...
async function retrieveImages(count, offset, destination, page) {
  ...
  while (imageUrls.size < count) {
    ...
    for (const imageResultsElement of imageResultsElements) {
      ...
      await imageResultsElement.click();
      // await page.waitForSelector(selectors.fullImageContainer, {visible: true});
      await page.waitForFunction(imageResultsElement => imageResultsElement.href.length > 0, {}, imageResultsElement);
      const imageElementLink = await imageResultsElement.evaluate(imageResultsElement => imageResultsElement.href);
      const imageUrl = /.+?imgurl=(.+?)(?:&.*$|$)/.exec(imageElementLink)[1]
        .replace(/%3A/g, ':')
        .replace(/%2F/g, '/');
