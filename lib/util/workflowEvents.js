const objectVerification = require('./objectVerification');
...
function workflowEvents(options) {
  this.init = function (scope) {
        ...
        if (ev.transition === 'verify') {
          return new Promise((resolve, reject) => {
            options.dataRepo.getItem(ev.item, null, {})
              .catch(reject)
              .then((objectItem) => {
                objectVerification(objectItem, options.dataRepo)
                  .catch(reject)
                  .then(() => resolve(true));
              });
          });
    ...
    options.workflows.on(
      [`modelTraining@${ION_NAMESPACE}.training`],
      (ev) => {
        if (ev.transition === 'train') {
          return new Promise((resolve, reject) => {
            const modelSnapshotItem = ev.item;

            (new trainModel(options, resetModelState)).exec({}, {
              params: {
                class: modelSnapshotItem.base._class,
                id: modelSnapshotItem.id
              }
            }).catch((err) => {
              resetModelState(modelSnapshotItem);
              reject(err);
            }).then(() => {
              resolve(true);
            });
        ...
        function resetModelState(modelSnapshotItem) {
          return options.workflows.pushToState(
            modelSnapshotItem,
            `modelTraining@${ION_NAMESPACE}`,
            'ready'
          );
        }
      }
