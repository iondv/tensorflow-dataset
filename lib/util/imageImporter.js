...
async function fromItemFiles(itemFiles, namespace, options) {
  const imageFiles = await itemFiles.filter((itemFile) => {
    const filePathParts = itemFile.name.split('.');
    const fileType = filePathParts[filePathParts.length - 1];
    return ALLOWED_IMAGE_TYPES.includes(fileType)
  });
  ...
  for (const imageFile of imageFiles) {
    ...
    importPromises.push(new Promise(async (resolve, reject) => {
      const objectItem = await options.dataRepo.createItem(`${OBJECT_ITEM_CLASS}@${namespace}`, {
        name: options.prefix ? `${options.prefix} #${imageNum}` : `#${imageNum}`,
        image: imageFile.id,
        state: 'learn',
        verified: false
      });
