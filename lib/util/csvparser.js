...
function parseCsvPartial(csvPath, count, offset = 0, options = {}) {
  ...
  let records;
  if (options.random) {
    const randomOffset = Math.floor(Math.random() * (csv.length - 2 - count));
    records = getRecords(csv.slice(randomOffset + 1, randomOffset + 2 + count), headers);
  }
  else
    records = getRecords(csv.slice(offset + 1 , offset + 2 + count), headers);
  return {
    headers,
    records
  };
