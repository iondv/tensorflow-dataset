...
function processWithTensorflow(options) {
        ...
        .then((predictionItem) => {
          if (predictionItem.base.modelSnapshot) {
            ...
            options.dataRepo.getItem(snapshotClass, snapshotId)
              .catch(err => reject(err))
              .then((snapshotItem) => {
                if (snapshotItem.base.typesToLearn && (snapshotItem.base.typesToLearn.length > 0)) {
                  const labelClass = `${snapshotItem.properties.typeLabels.meta.itemsClass}@${snapshotItem.classMeta.plain.namespace}`;
                  const typeLabels = [];

                  const labelReview = new Promise((resolve, reject) => {
                    if (snapshotItem.base.typesToLearn.length !== snapshotItem.base.typeLabels.length) {
                      modelUtils.reviewLabels(snapshotItem, options.dataRepo)
                        .catch(err => reject(err))
                        .then(reviewObject => resolve(reviewObject.modelSnapshotItem));
                    } else
                  ...
                  reject('the model is not configured to recognize any types.')
                }
              });
          }
        });
  ...
  async function processObject(itemClass, itemId, labeledTypes, dataRepo) {
    ...
    const imgStream = (await objectItem.files.image.getContents()).stream;
